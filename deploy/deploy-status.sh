...
# Source .env (may have unset vars, so don't use -u)
...
# Get health status
API_HEALTH=$(curl -sf "http://api-${ACTIVE}:3000/health" 2>/dev/null || echo '{"status":"unhealthy"}')
INGEST_HEALTH=$(curl -sf "http://ingest-${ACTIVE}:3002/health" 2>/dev/null || echo '{"status":"unhealthy"}')
