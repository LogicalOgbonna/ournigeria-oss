...
// ── LGA name → code lookup (loaded once from DB) ─────────────
let lgaLookup: Map<string, string> | null = null;

async function loadLgaLookup(): Promise<Map<string, string>> {
  if (lgaLookup) return lgaLookup;
  ...
  for (const e of entities) {
    ...
    lgaLookup.set(key, e.code);
    const normKey = `${e.lga.stateCode}:${normalizeLgaName(e.lga.name)}`;
    lgaLookup.set(normKey, e.code);
  }
  return lgaLookup;
}

function resolveLgaCode(stateCode: string, lgaName: string): string | null {
  if (!lgaLookup) return null;
...
async function ensureDocument(year: string, month: string): Promise<string> {
  ...
  const absolutePath = resolve(SOURCE_DIR, year, month, 'faac_disbursement.xlsx');
...
function parseLgas(ws: XLSX.WorkSheet): Array<{
  entityCode: string;
  netStatutory: number;
  ...
  for (let r = dataStart; r < 500; r++) {
    ...
    if (leftHalf) {
      ...
      if (!isTotalOnly) {
        const lgaRow = extractLgaFromHalf(ws, r, leftHalf, leftState);
        if (lgaRow) {
          results.push(lgaRow);
          emptyCount = 0;
        }
    ...
    if (rightHalf) {
      ...
      if (!isTotalOnly) {
        const lgaRow = extractLgaFromHalf(ws, r, rightHalf, rightState);
        if (lgaRow) {
          results.push(lgaRow);
          emptyCount = 0;
        }
...
function extractLgaFromHalf(
  ...
  const entityCode = resolveLgaCode(stateCode, lgaName);
  if (!entityCode) {
...
async function seedMonth(year: string, month: string): Promise<{
  ...
  const filePath = join(SOURCE_DIR, year, month, 'faac_disbursement.xlsx');
  ...
  if (!sumSheet) {
    console.error(`  ❌ ${year}/${month}: Missing 'Sum & FG' sheet`);
    return { created: false, skipped: true, states: 0, lgas: 0, fgnDetails: 0 };
  }
  ...
  if (!dates) {
    console.error(`  ❌ ${year}/${month}: Could not parse revenue/disbursement dates from title`);
    return { created: false, skipped: true, states: 0, lgas: 0, fgnDetails: 0 };
  }
  ...
  if (existing) {
    console.log(`  ⏭  ${year}/${month} (${dates.disbursementYear}-${dates.disbursementMonth}) already exists, skipping`);
    return { created: false, skipped: true, states: 0, lgas: 0, fgnDetails: 0 };
  }
  ...
  const lgaRows = lgaSheet ? parseLgas(lgaSheet) : [];

  if (dryRun) {
    console.log(`  🔍 ${year}/${month}: rev=${dates.revenueYear}-${dates.revenueMonth}, dis=${dates.disbursementYear}-${dates.disbursementMonth}`);
    console.log(`     Table I rows: ${tableI?.rows.length ?? 0}, FGN details: ${fgnDetails.length}, States: ${stateRows.length}, LGAs: ${lgaRows.length}`);
    return { created: false, skipped: false, states: stateRows.length, lgas: lgaRows.length, fgnDetails: fgnDetails.length };
  }

  const documentId = await ensureDocument(year, month);
...
async function main() {
  ...
  await loadLgaLookup();
  console.log(`  Loaded ${lgaLookup!.size / 2} LGA codes from database\n`);
