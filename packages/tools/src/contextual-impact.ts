...
export const contextualImpactInputSchema = z.object({
  amount: z
    .number()
...
export const contextualImpactOutputSchema = equivalentSchema;
...
export async function executeContextualImpact(input: z.infer<typeof contextualImpactInputSchema>) {
  const { amount, sector: _sector, state: _state, year: _year, domain, topic: _topic } = input;
