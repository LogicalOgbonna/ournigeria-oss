...
export const traverseGraphDescription = `Execute a read-only Cypher query against the knowledge graph for complex relationship queries.
...
export async function executeTraverseGraph(input: z.infer<typeof traverseGraphInputSchema>) {
  const { cypher: rawCypher, params: queryParams } = input;
