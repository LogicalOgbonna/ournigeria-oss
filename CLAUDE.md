...
# Start services
pnpm api:dev              # NestJS API — dev: http://localhost:3000/api. prod: https://api.ournigeria.ng/api
pnpm web:dev              # Next.js frontend — dev: http://localhost:3001. prod: https://app.ournigeria.ng to test the frontend
pnpm ingest:dev           # Ingestion pipeline — dev: http://localhost:3002/api/ingest. prod: https://ingest.ournigeria.ng/api/ingest
...
pnpm socials:dev          # X/Twitter automation on :3005 (internal service — no public domain; review drafts in the dashboard reply queue)
...
> **Dev vs prod URLs:** dev runs **locally** — hit each app on its `localhost` port from `pnpm <app>:dev` (there is no separate deployed dev environment). The `*.ournigeria.ng` domains are the live production deployment. Only a merge to `main` → `prod` advances production.
...
Known gotchas: the roamer leader-election loop dies after each blue/green deploy (recover via `POST /v1/roam/start`); X refresh tokens are single-use/rotating (re-auth via `pnpm x-oauth-authorize`); socials needs `EMBEDDING_BASE_URL` in its `/socials` Infisical path or RAG silently fails.
...
Opaque, server-stored session tokens (OWASP A07 remediation):
...
All features and new code MUST be tested using the local dev test user, against your **local** API
on `localhost:3000` — never against a deployed environment.

```bash
# Authenticate as the local test user
...
To browse the web UI with `/browse` or Playwright, cookies must be set on **both** origins (web
origin for Next.js middleware, API origin for cross-origin API calls):
...
The test user is created automatically on first dev-login. Use it for all automated testing,
evaluation runs, and feature validation.

> **`DevAuthModule` must never be registered outside local development.** It is wired in
> `apps/api/src/app.module.ts` behind an environment check; if that variable is unset the module
> loads, and `/api/auth/dev-login` will mint a real 30-day session for any unauthenticated caller.
> Verify the endpoint returns 404 on every deployed environment after each deploy.

## Planning
- Save all plans to the `.agent/plans/` folder (gitignored — working notes are kept out of this repo)
...
## Development Flow
1. **Plan** - Create a detailed plan and save it to `.agent/plans/` (gitignored)
