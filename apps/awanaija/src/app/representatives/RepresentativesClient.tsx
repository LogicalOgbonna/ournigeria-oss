...
export function RepresentativesClient({
  ...
  const sortedChain = [...initialChain].sort((a, b) => {
  ...
  const breadcrumb = [
    ...
    initialLocation?.lgaName,
    initialLocation?.wardName,
  ]
    .filter(Boolean)
    .join(" > ");
  ...
  function buildScope(scope?: Record<string, string>) {
    return {
      ...(scope || {}),
      ...(initialLocation?.stateCode ? { stateCode: scope?.stateCode || initialLocation.stateCode } : {}),
      ...(initialLocation?.stateName ? { stateName: initialLocation.stateName } : {}),
      ...(initialLocation?.lgaCode ? { lgaCode: scope?.lgaCode || initialLocation.lgaCode } : {}),
      ...(initialLocation?.lgaName ? { lgaName: initialLocation.lgaName } : {}),
      ...(initialLocation?.wardCode ? { wardCode: scope?.wardCode || initialLocation.wardCode } : {}),
      ...(initialLocation?.wardName ? { wardName: initialLocation.wardName } : {}),
    };
  }

  return (
    <main className="flex-1 max-w-2xl mx-auto px-4 pt-24 pb-20 w-full">
      {/* Header */}
      <div className="mb-6">
        <Link
          href="/"
        ...
        {initialLocation ? (
          <div className="mt-2">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              {breadcrumb || initialLocation.stateName}
            </p>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            {loading ? "Finding your representatives..." : "Select your location to see who represents you"}
          </p>
        )}
      </div>
