...
function SecTitle({ title, count, red }: { title: string; count: number; red?: boolean }) {
  return (
    <div className="flex items-center gap-4 mt-11 mb-[18px] max-md:mt-10 max-md:mb-3.5 max-md:gap-3">
      <h2 className={`font-serif italic font-normal text-[28px] max-md:text-[23px] m-0 ${red ? "text-red-500" : "text-slate-100"}`}>{title}</h2>
      <div className={`flex-1 h-px ${red ? "bg-red-500/30" : "bg-[oklch(0.28_0.012_160)]"}`} />
      <span className={`font-mono text-[10px] uppercase tracking-[0.14em] ${red ? "text-red-400" : "text-slate-500"}`}>{count}</span>
    </div>
  );
}
...
function ContactPills({ official }: { official: Official }) {
...
function ProvRow({ children, v }: { children: React.ReactNode; v: ProvFields }) {
  return (
    <div className="py-3.5 border-b border-[oklch(0.22_0.01_160)] last:border-0">
      {children}
      <div className="mt-1.5 flex gap-2.5 items-center flex-wrap"><VChips v={v} /><Sources evidence={v.evidence} /></div>
    </div>
  );
}
...
export function MagazineProfile({ official }: { official: Official }) {
  ...
  // pull-stat band — only the stats that have data
  ...
  const stats: { n: string; l: string; cls?: string }[] = [];
  if (elections.length) stats.push({ n: `${electionsWon} / ${elections.length}`, l: "elections won" });
  if (positions.length) stats.push({ n: String(positions.length), l: "public offices" });
  if (latestAsset != null) stats.push({ n: formatNaira(latestAsset)!, l: "assets declared", cls: "text-amber-400" });
  if (legal.length + corruption.length) stats.push({ n: String(legal.length + corruption.length), l: "legal records", cls: "text-red-500" });
          ...
          <div className="grid mt-[30px] border-y border-[oklch(0.28_0.012_160)] max-md:grid-cols-2" style={{ gridTemplateColumns: `repeat(${stats.length}, minmax(0, 1fr))` }}>
            {stats.map((s, i) => (
              <div key={s.l} className={`py-[22px] px-[18px] max-md:py-[18px] max-md:px-3 text-center ${i ? "border-l border-[oklch(0.28_0.012_160)] max-md:border-l-0" : ""} ${i % 2 ? "max-md:border-l max-md:border-[oklch(0.28_0.012_160)]" : ""} ${i > 1 ? "max-md:border-t max-md:border-[oklch(0.28_0.012_160)]" : ""}`}>
                <div className={`font-serif text-[38px] max-md:text-[32px] ${s.cls ?? "text-emerald-400"}`}>{s.n}</div>
                <div className="font-mono text-[9.5px] max-md:text-[9px] uppercase tracking-[0.14em] text-slate-500">{s.l}</div>
              </div>
            ))}
          </div>
        )}
