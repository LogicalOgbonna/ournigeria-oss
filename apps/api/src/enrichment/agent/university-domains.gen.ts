/**
 * GENERATED — world-university domains on GENERAL TLDs (.de/.ca/.fr/.fi/...).
 *
 * Source: https://github.com/Hipo/university-domains-list
 *         (world_universities_and_domains.json, fetched 2026-08-28)
 * Filter: domains already covered by the academic patterns (*.edu, *.edu.<cc>,
 *         *.ac.<cc>) are EXCLUDED — this file carries only the remainder that
 *         cannot be pattern-trusted (countries without an academic TLD).
 * Why: senators/HoR members with foreign degrees are rarely press-covered, so
 *      the trusted-press path does not reach them; the university's own site
 *      must be able to stand as an authoritative education source.
 * Regenerate by re-running the fetch+filter (see PR feat/world-university-domains).
 */
export const WORLD_UNIVERSITY_DOMAINS: string[] = [
  "a-aarhus.dk",
  "a2.keio.jp",
  "aaidu.org",
  "aalto.fi",
